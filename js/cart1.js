...
function calcul(kanap, local) {
  ...
  // ecoute on change
  for (let k = 0; k < quantitySelected.length; k++) {
    quantitySelected[k].addEventListener('change', (changeE) => {
      let idToChange = {
        id: local[k].kanapId,
        color: local[k].kanapColor,
        quantity: changeE.target.value
      }
